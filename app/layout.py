...
def layout(app):
    ...
    with app.tabbedFrame('Tabs'):
        ...
        with app.tab('Schedule'):
            ...
            app.addLabel('availableTime', row=3, column=0, colspan=2)
            app.addButtons(['Update Default', 'Use Once'], Plan.update_schedule, row=4, column=0, colspan=2)
    ...
    with app.subWindow('Schedule Setter'):
        ...
        with app.frame('schedule_setter_entry_box'):
            ...
            app.getEntryWidget('schedule_setter_hour').config(font=font_glance)
            app.addLabel('schedule_setter_colon', ':', row=0, column=1)
            app.addEntry('schedule_setter_minute', row=0, column=2)
            app.getEntryWidget('schedule_setter_minute').config(font=font_glance)
